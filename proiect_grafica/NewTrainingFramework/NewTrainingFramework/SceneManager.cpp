...
void SceneManager::parseSceneManagerXML() {
    ...
    for (xml_node<>* cameraNode = camerasNode->first_node("camera"); cameraNode; cameraNode = cameraNode->next_sibling())
    {
        ...
        Vector3 cameraPosition = Vector3(atof(cameraNode->first_node("position")->first_node("x")->value()), atof(cameraNode->first_node("position")->first_node("y")->value()), atof(cameraNode->first_node("position")->first_node("z")->value()));
       /* c.position.x = cameraNode->first_node("position")->first_node("x")->value();
        ...
        Vector3 cameraTarget = Vector3(atof(cameraNode->first_node("target")->first_node("x")->value()), atof(cameraNode->first_node("target")->first_node("y")->value()), atof(cameraNode->first_node("target")->first_node("z")->value()));
       /* c.target.x = cameraNode->first_node("target")->first_node("x")->value();
        ...
        Vector3 cameraUp = Vector3(atof(cameraNode->first_node("up")->first_node("x")->value()), atof(cameraNode->first_node("up")->first_node("y")->value()), atof(cameraNode->first_node("up")->first_node("z")->value()));
        ...
        GLfloat cameraTranslationSpeed = atof(cameraNode->first_node("translationSpeed")->value());
        GLfloat cameraRotationSpeed = atof(cameraNode->first_node("rotationSpeed")->value());
        GLfloat cameraFov = atof(cameraNode->first_node("fov")->value());
        GLfloat cameraNear = atof(cameraNode->first_node("near")->value());
        GLfloat cameraFar = atof(cameraNode->first_node("far")->value());
        Camera* c = new Camera(cameraPosition, cameraTarget, cameraUp);
        (*c).setType(cameraNode->first_node("type")->value());
        (*c).setTranslationSpeed(cameraTranslationSpeed);
        (*c).setRotationSpeed(cameraRotationSpeed);
        (*c).setFov(cameraFov);
        (*c).setNear(cameraNear);
        (*c).setFar(cameraFar);
        SceneManager::cameras.insert(std::pair<int, Camera*>(cameraId, c));
    }

    //activeCamera
    xml_node<>* activeCameraNode = sceneManagerNode->first_node("activeCamera");
    activeCamera = atoi(activeCameraNode->value());
    ...
    for (xml_node<>* objectNode = objectsNode->first_node("object"); objectNode; objectNode = objectNode->next_sibling())
    {
        ...
        if (xml_node<>* textureNode = objectNode->first_node("textures")) {
            ...
            o->texturesIdLength = texturesIdLength;
